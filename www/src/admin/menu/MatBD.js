...
export  function MatBD(menu, fun) {  
    ...
    this.iiiii
   /* this.gallery=new GalleryXZ(this.w, this.otstup, this._wh32 + this.otstup, function(ii){
        self.iiiii=ii
    ...
    if(this.param.materials_sorts){
        var newArr=[];
        for (var i = 0; i < this.param.materials_sorts.length; i++) {
            newArr[i] = this.param.materials_sorts[i].name;
        }
        this.comboSort.array = newArr;
        this.comboSort.arrayXZ = this.param.materials_sorts;
...
/*
    this.b=new DButton(this.w, this.w.width-30, 2, "+", function() {
        self.par.mInfo.setFunInput(
            "Создание матерьяла",
            "Валик ты точно хочеш новый матерьял",
            "Имя",
            function(){ 
                trace("yes",self.sort)
                self.creatMatXZ(self.par.mInfo.text)

            }
        );
    })
    this.b.width=this.b.height=28*/
